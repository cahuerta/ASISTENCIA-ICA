...
export async function irAPagoKhipu(datosPaciente) {
  ...
  if (typeof window !== 'undefined') sessionStorage.setItem('idPago', idPago);

  try {
    await guardarDatos(idPago, datosPaciente);
    const urlPago = await crearPagoKhipu({ idPago, datosPaciente, modoGuest: GUEST_DEFAULT === true ? true : false });
...
export async function simularPagoGuest(datosPaciente) {
  const d =
    datosPaciente || { nombre: 'Guest', rut: '99999999-9', edad: 30, dolor: 'Rodilla', lado: 'Izquierda' };
  const idPago = generarIdPago();
  if (typeof window !== 'undefined') sessionStorage.setItem('idPago', idPago);
...
export function leerRetornoPago() {
  ...
  if (estado === 'ok' && idPagoURL) {
    if (typeof window !== 'undefined') sessionStorage.setItem('idPago', idPagoURL);
