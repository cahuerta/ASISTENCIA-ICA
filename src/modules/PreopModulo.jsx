...
function sugerirCirugiaDesdeDolor(dolor=""){
  const s=(dolor||"").toLowerCase();
  if (s.includes("cadera")) return "Artroplastia total de cadera (ATC)";
  if (s.includes("rodilla")) return "Artroplastia total de rodilla (ATR)";
  return "";
}
...
export default function PreopModulo({ initialDatos }) {
  ...
  // Flujo
  // 'idle' | 'comorbilidades' | 'cirugia' | 'ia_cargando' | 'preview'
  ...
  const [tipoCirugia, setTipoCirugia] = useState(()=>{
    try{
      ...
      if (preset && preset.startsWith("Otro")) { return (otro.trim()||""); }
      return preset || "";
    }catch{return ""}
  });
  ...
  const handleContinuarPreop = async ()=>{
    ...
    const idPago = sessionStorage.getItem("idPago") || ("preop_"+Date.now()+"_"+Math.floor(Math.random()*10000));
    sessionStorage.setItem("idPago", idPago);
    sessionStorage.setItem("modulo","preop");
    sessionStorage.setItem("datosPacienteJSON", JSON.stringify({ ...datos, edad: edadNum }));

    // Sugerir tipo por dolor si no hay nada guardado
    try{
      const ya = sessionStorage.getItem("preop_tipoCirugia");
      if (!ya){
        const sug = sugerirCirugiaDesdeDolor(datos?.dolor||"");
        if (sug) sessionStorage.setItem("preop_tipoCirugia",sug);
      }
    }catch{}
  ...
  const handleEnviarComorbilidades = async (formData)=>{
    ...
    try{
      await fetch(`${BACKEND_BASE}/guardar-datos-preop`,{
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({
          idPago, datosPaciente:{...datos}, comorbilidades: limpio,
  ...
  // Paso 2: confirmar tipo
  const handleConfirmarCirugia = async ()=>{
    const idPago=sessionStorage.getItem("idPago");
    if(!idPago) return alert("ID de pago no encontrado");
    let seleccion=tipoCirugia;
    if(!seleccion) return alert("Seleccione el tipo de cirugía.");
    if(seleccion.startsWith("Otro") && !tipoCirugiaLibre.trim()) return alert("Especifique el tipo de cirugía en 'Otro'.");
  ...
  const llamarIAyConstruirPreview = async (tipoSel, comorb)=>{
    ...
    try{
      const payload={
        idPago,
        paciente:{ nombre:datos?.nombre||"", rut:datos?.rut||"", edad:Number(datos?.edad)||null, dolor:datos?.dolor||"", lado:datos?.lado||"" },
      ...
      try{
        await fetch(`${BACKEND_BASE}/guardar-datos-preop`,{
          method:"POST", headers:{"Content-Type":"application/json"},
          body: JSON.stringify({ idPago, datosPaciente:{...datos}, comorbilidades: comorb||comorbilidades||{}, tipoCirugia: tipoSel||tipoCirugia||"", examenesIA: examenes, informeIA: informe })
        });
      }catch{}
      ...
      setExamenesIA([]); setInformeIA("No fue posible obtener el informe de IA en este momento.");
      setPaso("preview");
    }
  };

  // Paso 4: pagar desde preview
  const handlePagarDesdePreview = async ()=>{
    ...
    try{
      await fetch(`${BACKEND_BASE}/guardar-datos-preop`,{
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ idPago, datosPaciente:{...datos}, comorbilidades, tipoCirugia, examenesIA:Array.isArray(examenesIA)?examenesIA:[], informeIA: informeIA||"" })
      });
  ...
  const handleDescargarPreop = async ()=>{
    ...
    const intentaDescarga = async ()=>{
      ...
      a.href=url; a.download=`preop_${(datos?.nombre||"paciente").replace(/ /g,"_")}.pdf`;
      ...
      for(let i=1;i<=maxIntentos;i++){
        ...
        if(r.status===404){
          if(!reinyectado){
            ...
            await fetch(`${BACKEND_BASE}/guardar-datos-preop`,{
              method:"POST", headers:{"Content-Type":"application/json"},
              body: JSON.stringify({ idPago, datosPaciente: datosReinyectar, comorbilidades, tipoCirugia, examenesIA:Array.isArray(examenesIA)?examenesIA:[], informeIA: informeIA||"" })
            });
            reinyectado=true; await sleep(500); continue;
          } else {
            alert("No se pudo descargar el PDF después de reintentar."); break;
          }
        }
        alert("No se pudo descargar el PDF."); break;
      }
    }catch(error){
      console.error(error); alert("No se pudo descargar el PDF.");
    }finally{
      setDescargando(false); setMensajeDescarga("");
    }
  };
  ...
  const handleSimularPagoGuest = async ()=>{
    ...
    const datosGuest={ nombre:"Guest", rut:"99999999-9", edad:30, dolor:"Rodilla", lado:"Izquierda" };
    sessionStorage.setItem("idPago",idPago);
    sessionStorage.setItem("modulo","preop");
    ...
    url.searchParams.set("pago","ok"); url.searchParams.set("idPago",idPago);
  ...
  return (
    <div style={styles.card}>
      ...
      <div style={{ marginBottom:10, color: T.text }}>
        ...
        <div><strong>Motivo/Área:</strong> {`Dolor en ${(datos?.dolor||"")}${datos?.lado?` ${datos.lado}`:""}`.trim() || "—"}</div>
        {tipoCirugia ? <div><strong>Tipo de cirugía:</strong> {tipoCirugia}</div> : null}
      </div>
      ...
      {(paso==="preview" || (Array.isArray(examenesIA) && (examenesIA.length || informeIA))) && (
        <>
          {Array.isArray(examenesIA) && examenesIA.length>0
            ? <div><strong>Exámenes a solicitar (IA):</strong><ul style={{ marginTop:6 }}>{examenesIA.map((e,idx)=><li key={`${e}-${idx}`}>{typeof e==="string"?e:(e?.nombre || JSON.stringify(e))}</li>)}</ul></div>
            : <div style={{ marginTop:6, color: T.textMuted }}>(La IA no devolvió una lista de exámenes en esta ocasión.)</div>
          }
          {informeIA ? <div style={{ marginTop:8 }}><strong>Informe IA (resumen):</strong><div style={styles.informeBox}>{informeIA}</div></div> : null}
        </>
      )}

      {/* Controles */}
      {pagoRealizado ? (
        <button style={{ ...styles.btn, marginTop:12 }} onClick={handleDescargarPreop} disabled={descargando} title={mensajeDescarga || "Verificar y descargar"}>
        ...
        <>
          {paso==="idle" && (
            <>
              <button style={{ ...styles.btn, backgroundColor: T.primary, marginTop:12 }} onClick={handleContinuarPreop}
                title="Comorbilidades → (Tipo de cirugía si falta) → IA → Preview → Pago">
                Continuar (Pre Op)
              </button>
              <button style={{ ...styles.btn, backgroundColor: T.muted, marginTop:8 }} onClick={handleSimularPagoGuest}>
                Simular Pago (Guest)
              </button>
            </>
          )}
          ...
          {paso==="comorbilidades" && (
            <div style={styles.modal}>
              <div style={styles.modalCard}>
                ...
                <FormularioComorbilidades initial={comorbilidades || {}} onSave={handleEnviarComorbilidades} onCancel={()=>setPaso("idle")} />
                <div style={{ display:"flex", gap:8, marginTop:12 }}>
                  <button style={{ ...styles.btn, background: T.muted }} onClick={()=>setPaso("idle")}>Cerrar</button>
                </div>
              </div>
            </div>
          )}
                ...
                <div style={{ display:"grid", gap:8 }}>
                  {TIPOS_CIRUGIA.map((t)=>(
                    <label key={t} style={styles.radioRow}>
                      <input type="radio" name="tipoCirugia" value={t} checked={tipoCirugia===t} onChange={(e)=>setTipoCirugia(e.target.value)} />
                      <span style={{ marginLeft:8 }}>{t}</span>
                    </label>
                  ))}
                  {tipoCirugia?.startsWith("Otro") && (
                    <input placeholder="Especifique el tipo de cirugía" value={tipoCirugiaLibre} onChange={(e)=>setTipoCirugiaLibre(e.target.value)} style={styles.input} />
                  )}
                </div>
                <div style={{ display:"flex", gap:8, marginTop:12 }}>
                  <button style={{ ...styles.btn, background: T.muted }} onClick={()=>setPaso("comorbilidades")}>Volver</button>
                  <button style={{ ...styles.btn, background: T.primary }} onClick={handleConfirmarCirugia}>Confirmar</button>
                </div>
              </div>
            </div>
          )}
          ...
          {paso==="preview" && (
            <button style={{ ...styles.btn, backgroundColor: T.primary, marginTop:12 }} onClick={handlePagarDesdePreview}>
              Pagar ahora (Pre Op)
            </button>
          )}
        </>
...
function normalizarComorbilidades(c){
  if(!c) return {};
  return {
    hta: !!c.hta, dm2: !!c.dm2, dislipidemia: !!c.dislipidemia, obesidad: !!c.obesidad,
    tabaquismo: !!c.tabaquismo, epoc_asma: !!c.epoc_asma, cardiopatia: !!c.cardiopatia,
    erc: !!c.erc, hipotiroidismo: !!c.hipotiroidismo, artritis_reumatoide: !!c.artritis_reumatoide,

    alergias: (typeof c.alergias_flag!=="undefined" || typeof c.alergias_detalle!=="undefined")
      ? { tiene: !!c.alergias_flag, detalle: (c.alergias_detalle||"").toString() }
      : (c.alergias||"").toString(),

    anticoagulantes: { usa: !!c.anticoagulantes, detalle: (c.anticoagulantes_detalle||"").toString() },
...
/* Estilos */
const styles = {
  card:{ background: T.surface, borderRadius:8, padding:16, boxShadow: T.shadowSm, border:`1px solid ${T.border}`, color: T.text },
  btn:{ backgroundColor: T.primary, color: T.onPrimary || "#fff", border:"none", padding:"12px", borderRadius:"8px", fontSize:"16px", cursor:"pointer", width:"100%", boxShadow: T.shadowSm },
  informeBox:{ background: T.bg, border:`1px solid ${T.border}`, borderRadius:8, padding:12, marginTop:6, whiteSpace:"pre-wrap", color: T.text },
  modal:{ position:"fixed", inset:0, background: T.overlay || "rgba(0,0,0,0.35)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:9999, padding:12 },
  modalCard:{ background: T.surface, borderRadius:10, padding:16, width:"min(720px, 92vw)", boxShadow: T.shadowMd, border:`1px solid ${T.border}`, color: T.text },
  input:{ width:"100%", padding:"10px", border:`1px solid ${T.border}`, borderRadius:8, fontSize:14, background: T.surface, color: T.text },
  radioRow:{ display:"flex", alignItems:"center", padding:"6px 8px", border:`1px solid ${T.border}`, borderRadius:8, background: T.surface, color: T.text },
};
